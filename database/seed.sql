-- ═══════════════════════════════════════════════════════════════════════════
-- ASR FORM — Multi-SaaS Database Seed Data
...
-- ─── 1. System Roles (3 roles only: Super Admin=1, Admin=2, User=3) ───
-- Hindari collision duplicate key 'User'/'Admin'/'Super Admin' jika database lama memiliki id berbeda
UPDATE `roles` SET `name` = CONCAT(`name`, '_legacy_', `id`) WHERE `id` NOT IN (1, 2, 3) AND `name` IN ('Super Admin', 'Admin', 'User');
...
-- Migrasikan referensi role user lama jika ada yang pointing ke id != 3
UPDATE `users` SET `role_id` = 3 WHERE `role_id` NOT IN (1, 2, 3) OR `role_id` IS NULL;

-- Bersihkan role legacy lama (e.g. Manager, Staff, Guest, dsb)
DELETE FROM `roles` WHERE `id` NOT IN (1, 2, 3);

-- ─── 2. Default Super Admin (in admins table) ───
-- [PENTING]: Ganti password ini setelah login pertama kali!
-- Email: admin@asrform.app
-- Password Default: admin123 (bcrypt hash)
UPDATE `admins` SET `email` = CONCAT('admin_old_', `id`, '@asrform.app') WHERE `email` = 'admin@asrform.app' AND `id` != 1;

INSERT INTO `admins` (`id`, `name`, `email`, `phone`, `password`, `role_id`, `status`, `plan`) VALUES
(1, 'Super Administrator', 'admin@asrform.app', '', '$2y$10$L0fzbd.MYelfV0cxmO5eDuOE.1aJGoosIkUXSFkXjkeFTI/uunm62', 1, 'active', 'Enterprise')
ON DUPLICATE KEY UPDATE `role_id` = 1, `status` = 'active', `email` = VALUES(`email`);
...
('page_about_content', '<h2>Apa itu ASR FORM?</h2>\n<p>ASR FORM adalah platform berbasis web untuk membuat formulir digital dan mengubah data yang terkumpul menjadi dokumen resmi secara otomatis.</p>'),
...
('payment_qris_image', ''),
('payment_qris_instructions', 'Pindai kode QRIS di atas melalui mobile banking atau e-wallet.'),
('payment_transfer_enabled', '1'),
('payment_bank_accounts', '[{"bank":"BCA","number":"1234567890","holder":"ASR FORM DIGITAL"},{"bank":"Mandiri","number":"0987654321","holder":"ASR FORM DIGITAL"}]'),
('payment_transfer_instructions', 'Silakan lakukan transfer ke salah satu rekening resmi kami di atas.'),

-- WhatsApp Gateway (Global URL — managed by Super Admin)
('wa_gateway_url', 'https://gateway.asr-desain.my.id'),
('wa_enabled', '0'),
('wa_api_key', ''),
('wa_sender', ''),
('wa_admin_number', ''),
...
-- ─── 5. Default Numbering Configs (for Super Admin tenant) ───
INSERT INTO `numbering_configs` (`admin_id`, `name`, `format`, `code`, `current_sequence`, `reset_period`) VALUES
(1, 'Surat Keterangan', '{{sequence}}/SK/{{roman_month}}/{{year}}', 'SK', 0, 'yearly'),
(1, 'Surat Tugas', '{{sequence}}/ST/{{roman_month}}/{{year}}', 'ST', 0, 'yearly')

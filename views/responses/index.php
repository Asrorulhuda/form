...
<div class="flex items-center justify-between mb-4 flex-wrap gap-3">
    <div>
        <h2 style="font-size: 20px; font-weight: 800; color: var(--text-primary); margin: 0 0 4px;">
            <?= e($title ?? 'Data Respons Formulir') ?>
        </h2>
        <p class="text-sm text-muted" style="margin: 0;">
            Total: <strong><?= $total ?></strong> data respons tersimpan di sistem.
        </p>
    </div>

    <div class="flex gap-2 flex-wrap items-center">
        <!-- Export Excel -->
        <a href="<?= url('responses/export' . ($selectedForm ? "?form_id={$selectedForm}" : '')) ?>" class="btn btn-primary btn-sm" style="background: #16a34a; border-color: #16a34a; font-weight: 700;" title="Unduh data respons dalam format Excel (.xlsx)">
        ...
        <?php if ($total > 0): ?>
            <form action="<?= url('responses/clear') ?>" method="POST" style="display: inline;" onsubmit="return confirm('PERINGATAN: Apakah Anda yakin ingin MENGHAPUS SEMUA data respons <?= $selectedForm ? 'pada formulir yang dipilih' : 'di seluruh sistem' ?>? Tindakan ini tidak dapat dibatalkan!');">
                ...
                <?php endif; ?>
                <button type="submit" class="btn btn-danger btn-sm" title="Hapus seluruh data respons">
                    🗑️ Kosongkan Data
                </button>
            </form>
        <?php endif; ?>
...
<div class="card mb-4">
    <div class="card-body" style="padding: 14px 20px;">
        <form method="GET" action="<?= url('responses') ?>" class="flex items-center gap-2 flex-wrap">
            <label class="text-xs text-muted" style="font-weight: 700;">Filter Formulir:</label>
            <select name="form_id" class="form-control form-control-sm" style="max-width: 300px; font-size: 13px;">
            ...
            <?php if ($selectedForm): ?>
                <a href="<?= url('responses') ?>" class="btn btn-soft-primary btn-sm">&times; Reset</a>
            <?php endif; ?>
        </form>
    </div>
</div>
...
<div class="card">
    <div class="table-container">
        <table class="table">
            <thead>
                <tr>
                    <th style="width: 100px;">ID Respons</th>
                    <th>Formulir</th>
                    <th>Responden</th>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <div class="flex gap-2 text-xs text-muted mt-1">
                                    <a href="<?= url("forms/{$r->form_id}/responses") ?>" style="color: var(--primary-600); text-decoration: none; font-size: 11px;">
                    ...
                    <tr>
                        <td colspan="7">
                            <div class="empty-state" style="padding: 48px 20px;">
                                ...
                                <p class="empty-state-desc">Respons dari pengguna yang mengisi formulir publik akan otomatis tampil di sini.</p>
    ...
    <!-- Pagination -->
    <?php if ($lastPage > 1): ?>
        <div class="card-footer" style="padding: 12px 20px; background: #fafafa; border-top: 1px solid var(--border-subtle); display: flex; align-items: center; justify-content: space-between;">
